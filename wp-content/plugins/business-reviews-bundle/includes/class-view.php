...
class View {
    ...
    private function render_badge2($businesses, $reviews, $options) {
        ...
        ">
            ...
            >
                ...
                <?php } if ($options->view_mode != 'badge_inner') { echo '<div class="rplg-badge2-close">×</div>'; } ?>
            </div>
            <?php } ?>
        </div>
        <?php
    ...
    private function rating_temp($businesses, $reviews, $options) {
        foreach ($businesses as $business) {
            ...
            $temp = str_replace('{{reviews_url}}', $this->get_allreview_url($business), $temp);
    ...
    private function anchor($url, $class, $text, $open_link, $nofollow_link, $onclick = '', $title = '') {
        ?><a href="<?php echo $url; ?>" class="<?php echo $class; ?>" <?php if ($open_link) { ?>target="_blank"<?php } ?> <?php if ($nofollow_link) { ?>rel="nofollow"<?php } ?> <?php if (strlen($onclick) > 0) { ?>onclick="<?php echo $onclick; ?>"<?php } ?> <?php if ($this->_strlen($title) > 0) { ?>title="<?php echo $title; ?>"<?php } ?>><?php echo $text; ?></a><?php
    }
    ...
    private function trim_text($text, $size) {
        if ($size > 0 && $this->_strlen($text) > $size) {
            ...
            if ($idx > 0) {
                $visible_text = $this->_substr($text, 0, $idx);
                $invisible_text = $this->_substr($text, $idx, $this->_strlen($text));
            }
            echo $visible_text;
